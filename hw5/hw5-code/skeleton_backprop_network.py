...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        L = self.num_layers
        z, v = self.forward_pass(x)
        deltas = self.backward_pass(v, y)
        dw = []
        for l in range(L - 1):    
            dw.append(
                np.dot(deltas[l], np.transpose(z[l]))
            )
        return deltas, dw
    ...
    def forward_pass(self, x):
        ...
        v = []
        for l in range(L-1):
            v.append(np.array(self.weights[l]).dot(z[-1]) + np.array(self.biases[l]))
            z.append(relu(v[-1]))
        return z, v 
    ...
    def backward_pass(self, v, y):
        ...
        for l in reversed(range(L-2)):
            deltas.insert(0, np.dot(np.transpose(self.weights[l+1]), deltas[0]) * relu_derivative(v[l]))   
        return deltas
